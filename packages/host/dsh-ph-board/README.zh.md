...
- 每次请求一个 Python 子进程，冷 import `board.store`。在小型 store 上以人类节奏轮询完全够用；若实测轮询延迟成为问题，再升级为常驻读 worker。
