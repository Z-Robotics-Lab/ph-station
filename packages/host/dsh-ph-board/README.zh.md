...
physical-harness fork 的主机侧桥接包。一个 Typert Remote（`board`），把 ph-station 的面板转发到主机（motherboard）的证据层（`board/store.py`），经其 CLI 面（`board/storecli.py`）访问，同源部署在 gateway 的 `trusted-host` 防线之后。两个方法会写：`submitBrief`——经 storecli 把 brief 原子投递进运行时会话的 inbox，以及 `modelServer`——本地模型服务进程的启停。其余全部只读。
