...
- **wiki graph** (center) — the `@xyflow/react` canvas (one global dagre left→right pass, per-kind silhouettes and hues, edges colored per relation, labels under the cursor or on the selection's edges) drawing the selection's neighborhood and refitting the viewport after every change: no selection → the class overview (class nodes sized by skill count, labeled `grasp · 14`, the benchmark / package nodes a class touches, and one aggregated DEPENDS_ON / BOUND_TO / EVIDENCED_ON edge per class pair labeled with its fold-edge count — no skill nodes); a class → itself, its generic skills (no INSTANCE_OF out-edge) and their DEPENDS_ON / BOUND_TO / EVIDENCED_ON neighbors, instances collapsed under their generic behind a `+n` badge that unfolds them; a library skill → its depth-1 neighbors, depth 2 behind the "One level deeper" toggle; any other node → its depth-2 neighborhood (never a class's members through IN_CLASS). A single click selects; the tree and detail follow.
