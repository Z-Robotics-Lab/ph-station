...
- **片段只有路径，没有图。** `rsiFrames` 返回会话相对路径，board 没有面提供字节，所以关键片段只列路径（▶/▣ 标记 mp4/gif）；缩略图与内联视频等一个提供字节的面。
- **campaign 列表跟随运行时事件流。** 列表（页面上的和侧栏 RSI 卡里的）靠对每次启动内 `runtimeEvents` 里出现的任务逐个调 `rsiRun` 得到，上次重启前认领的 campaign 要等本页再次启动该任务才会出现；一个列 campaigns 目录的面可以补上这个缺口。
...
- **每次轮询的 board 调用。** 侧栏每 15s 轮询驱动七次以上 board 读取（sessions、session、session_progress、runtime_status、runtime_events、stores，再为进化态会话的最新 campaign 读 runtime_events + rsi_run），每次都是一个冷的 `board.storecli` 子进程。在小型 store 上以人类节奏没问题；若实测轮询延迟成为问题，再合并成单个读取函数。
