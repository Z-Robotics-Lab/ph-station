...
纯消费者：每个状态都从 board 载荷原样拷贝；折叠层（`src/client/graph.ts`）只组装渲染状态，不做任何计算（宪章：TS 只渲染）。事件轮询在上次请求结束后按执行中约 1.2s、空闲约 4s 的间隔执行；首次后续轮询固定为 1.2s。首次加载不受隐藏状态影响，后续刷新在隐藏时暂停。封存详情独立读取，在上次请求结束后约 16s 刷新，历史缓慢不会阻塞实时事件。切换会话或卸载后丢弃旧请求结果。取景窗面板则对 board 的 `runtimeFrame` 面做长轮询（`afterTs` 游标 + 约 0.9s 的 `waitMs` 服务端阻塞），回复一到立刻重发，到手帧率因此跟上 harness 的转储帧率。过程流面板会给磁盘上有对应图片的行挂一张关键帧缩略图（`runs/<session>/keyframes/<seq:06d>-<kind>.jpg`，由 `harness.opstream` 写入，并在 `opstream.arm()` 时与 feed 一起清空）：它轮询不含字节的索引（`runtimeKeyframes`，约 3s），只有当某行缩略图首次进入滚动视口或被点击时才去取该行的 JPEG（`runtimeKeyframe`），绝不批量拉取。点击后弹出灯箱，←/→ 在本次运行的其他关键帧行之间切换（捕获阶段监听，不惊动 scrubber 快捷键），Esc 关闭。行上本来就有的事件 `seq` 就是全部关联键——没有关键帧的会话渲染结果与从前完全一致。

会话范围：面板自动跟随最新的实时运行时会话。首批事件立即显示当前任务并隐藏已完成历史；后续挂载复用该对话的事件起点。表头选择器按 board 自己的 `runtime.boot` 标记把发现到的会话分成「实时运行时」与「历史归档」两组；手动选中的会话则完整重放。
...
- feed 的「按对话」起始 seq 只存在页面内存里：刷新或新开标签页会重新发现当前任务，并隐藏已完成历史。要持久化需要一个面板并不拥有的存储。
