...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-ph-battle': { kind: 'none', reason: 'Browser-side read-only panel over board Remote state; registers nothing model-facing.' },
  'packages/client/ui-ph-dash': { kind: 'none', reason: 'Browser-side dockview arrangement of existing conversation.view panels; registers nothing model-facing.' },
  'packages/client/ui-ph-icons': { kind: 'none', reason: 'Browser-side presentational icon atoms; registers nothing model-facing.' },
  'packages/client/ui-ph-livegraph': { kind: 'none', reason: 'Browser-side read-only panel over the board runtimeEvents feed; registers nothing model-facing.' },
  'packages/client/ui-ph-ops': { kind: 'none', reason: 'Browser-side read-only operator surfaces over board Remote state; registers nothing model-facing.' },
  'packages/client/ui-ph-panels': { kind: 'none', reason: 'Browser-side read-only panels over board Remote state; the 任务台 chips only prefill an operator-sent composer draft.' },
  'packages/client/ui-ph-vault': { kind: 'none', reason: 'Browser-side read-only wiki over the board vault graph; the agent reads the same vault through the board MCP/CLI faces.' },
  'packages/host/dsh-ph-board': { kind: 'none', reason: 'Read-only Host Remote serving the browser panels; the chat LLM reads the same board.store functions through the MCP server this package does not own.' },
