...
[`@deepseek-ai/dsh-ph-board`](../../packages/host/dsh-ph-board) is this fork's host bridge from the ph-station panels to the physical-harness evidence layer (`board/store.py`) and planning face (`board/planning.py`). Every `ctx.board` method runs the harness's CLI face (`board/storecli.py`) via `execFile` and forwards its stdout JSON verbatim — zero statistics, zero interpretation — so a panel renders the byte-identical dict the chat LLM receives through the MCP face of the same function. Reads dominate; `submitSkillPlan` and `cancelBrief` are the harness's own brief lifecycle writes, re-verified and dropped harness-side.
