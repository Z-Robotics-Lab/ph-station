...
flowchart LR
  ...
  pkg_ph_board["ph-board"]
  svc_board["ctx.board<br/>Physical-harness board bridge"]
...
| `ctx.board` | `core` | [`ph-board`](../packages/host/dsh-ph-board) | - | - | - | 主机（motherboard）仓库 board.storecli 之上的只读 Host Remote；PH 浏览器面板经 Remote BFF 读取 campaign、会话、运行时事件与 vault 投影，无任何主机侧 consumer。 |
